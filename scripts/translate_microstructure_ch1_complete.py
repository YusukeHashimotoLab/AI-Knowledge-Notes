...
Handles large HTML files systematically while preserving structure
"""
...
def get_comprehensive_translations() -> Dict[str, str]:
    """
    Comprehensive translation mappings for materials microstructure chapter 1
    ...
    # ===== Section 1.2: Types and Properties of Grain Boundaries =====
    translations.update({
    ...
    # ===== Section 1.3: Hall-Petch Relationship =====
    translations.update({
    ...
    translations.update({
        ...
        '<strong>解説</strong>: Hall-Petch関係は$d^{-1/2}$に対して線形です。細粒化による強化効果を定量的に評価でき、目標強度を達成するための粒径を逆算することもできます。':
            '<strong>Explanation</strong>: The Hall-Petch relationship is linear against $d^{-1/2}$. The strengthening effect by grain refinement can be quantitatively evaluated, and the grain size to achieve target strength can be calculated.',
    })

    # ===== Code Example 3: Misorientation Distribution =====
    translations.update({
        ...
        '<strong>解説</strong>: ランダムな結晶方位を持つ材料では、方位差はMackenzie分布に従います。実際の材料では、Σ3双晶境界などの特殊粒界がピークとして現れます。':
            '<strong>Explanation</strong>: In materials with random crystallographic orientations, misorientation follows the Mackenzie distribution. In actual materials, special boundaries such as Σ3 twin boundaries appear as peaks.',
    })

    # ===== Code Example 4: CSL Boundaries =====
    translations.update({
...
def main():
    ...
    print("="*70)
    print("Materials Microstructure Chapter 1 Translation")
    print("="*70)
    ...
    print(f"\n{'⚠️  PARTIAL TRANSLATION' if translation_percentage < 95 else '✅ COMPLETE TRANSLATION'}")
    if translation_percentage < 95:
        print(f"   Note: This is a partial translation. {remaining_jp_count:,} Japanese characters remain.")
        print(f"   Additional translation mappings may be needed for complete translation.")
    print("="*70)
