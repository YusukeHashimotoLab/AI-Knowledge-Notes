...
# -*- coding: utf-8 -*-
"""
Complete translation script for 3D Printing Introduction Chapter 4
Translates Japanese HTML to English while preserving all structure
"""
...
def translate_chapter4():
    ...
    # Read source file
    source_path = '/Users/yusukehashimoto/Documents/pycharm/AI_Homepage/wp/knowledge/jp/MS/3d-printing-introduction/chapter-4.html'
    target_path = '/Users/yusukehashimoto/Documents/pycharm/AI_Homepage/wp/knowledge/en/MS/3d-printing-introduction/chapter-4.html'
    ...
    translations = {
        ...
        # Title
        '第4章：材料噴射法・結合剤噴射法・その他AM技術 - MS Terakoya': 'Chapter 4: Fundamentals of Additive Manufacturing - MS Terakoya',
        ...
        '積層造形（Additive Manufacturing, AM）とは、<strong>ISO/ASTM 52900:2021規格で定義される「3次元CADデータから材料を層ごとに積み上げて物体を製造するプロセス」</strong>です。従来の切削加工（除去加工）とは対照的に、必要な部分にのみ材料を付加するため、以下の革新的な特徴を持ちます：':
            'Additive Manufacturing (AM) is <strong>defined by ISO/ASTM 52900:2021 standard as "the process of manufacturing objects by adding material layer by layer from 3D CAD data"</strong>. In contrast to traditional subtractive machining (material removal), AM adds material only where needed, providing the following innovative characteristics:',
        ...
        # Info box - Industrial importance
        ...
        '<strong>1986年: ステレオリソグラフィ（SLA）発明</strong> - Chuck Hull博士（3D Systems社創業者）が光硬化樹脂を層状に硬化させる最初のAM技術を発明（US Patent 4,575,330）。「3Dプリンティング」という言葉もこの時期に誕生。':
            '<strong>1986: Stereolithography (SLA) Invention</strong> - Dr. Chuck Hull (founder of 3D Systems) invented the first AM technology that cures photopolymer resin layer by layer (US Patent 4,575,330). The term "3D printing" was also coined during this period.',

        '<strong>1988年: 選択的レーザー焼結（SLS）登場</strong> - Carl Deckard博士（テキサス大学）がレーザーで粉末材料を焼結する技術を開発。金属やセラミックスへの応用可能性を開く。':
            '<strong>1988: Selective Laser Sintering (SLS) Introduction</strong> - Dr. Carl Deckard (University of Texas) developed technology to sinter powder materials with laser. Opened possibilities for metals and ceramics applications.',

        '<strong>1992年: 熱溶解積層（FDM）特許</strong> - Stratasys社がFDM技術を商用化。現在最も普及している3Dプリンティング方式の基礎を確立。':
            '<strong>1992: Fused Deposition Modeling (FDM) Patent</strong> - Stratasys commercialized FDM technology. Established the foundation for the most widespread 3D printing method today.',

        '<strong>2005年: RepRapプロジェクト</strong> - Adrian Bowyer教授がオープンソース3Dプリンタ「RepRap」を発表。特許切れと相まって低価格化・民主化が進展。':
            '<strong>2005: RepRap Project</strong> - Professor Adrian Bowyer announced the open-source 3D printer "RepRap". Combined with patent expiration, this advanced cost reduction and democratization.',
        ...
        '<strong>2023年現在: 大型化・高速化の時代</strong> - バインダージェット、連続繊維複合材AM、マルチマテリアルAMなど新技術が産業実装段階へ。':
            '<strong>2023 Present: Era of Large-scale and High-speed</strong> - New technologies such as binder jetting, continuous fiber composite AM, and multi-material AM are entering industrial implementation phase.',
    ...
    for jp_text, en_text in translations.items():
        if jp_text in translated:
            translated = translated.replace(jp_text, en_text)
            translation_count += 1

    # Calculate Japanese character percentage
    jp_pattern = re.compile(r'[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF]+')
    jp_chars_before = len(''.join(jp_pattern.findall(content)))
    jp_chars_after = len(''.join(jp_pattern.findall(translated)))

    jp_percentage_before = (jp_chars_before / len(content)) * 100
    jp_percentage_after = (jp_chars_after / len(translated)) * 100
    ...
    print(f"Translations applied: {translation_count}")
    print(f"Japanese chars before: {jp_chars_before} ({jp_percentage_before:.2f}%)")
    print(f"Japanese chars after: {jp_chars_after} ({jp_percentage_after:.2f}%)")
    print(f"=" * 60)

    return jp_chars_after, jp_percentage_after

if __name__ == '__main__':
    jp_chars, jp_percent = translate_chapter4()
    print(f"\n✓ Final Japanese character count: {jp_chars}")
    print(f"✓ Final Japanese percentage: {jp_percent:.2f}%")
