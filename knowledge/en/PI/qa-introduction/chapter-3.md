---
title: "Chapter 3: Six Sigma and DMAIC Methodology"
chapter_title: "Chapter 3: Six Sigma and DMAIC Methodology"
subtitle: Dramatically Improve Quality with Data-Driven Problem Solving Framework
---

[AI Terakoya Top](<../index.html>)›[Process Informatics](<../../index.html>)›[QA](<../../PI/qa-introduction/index.html>)›Chapter 3

🌐 EN | [🇯🇵 JP](<../../../jp/PI/qa-introduction/chapter-3.html>) | Last sync: 2025-11-16
...
Sigma level represents the distance from the process mean to the specification limit, expressed as multiples of standard deviation (σ). Higher levels indicate better quality.

Sigma Level | DPMO | Yield | Quality Rating  
---|---|---|---  
2σ | 308,537 | 69.1% | Very Poor  
3σ | 66,807 | 93.3% | Poor  
...
Calculate DPMO and sigma level from defect data.
    ...
    # Simulate 4σ process (mean 50.0, Cpk=1.33)
    ...
    print(f"Short-term sigma level: {results['sigma_level_st']:.2f}σ")
    print(f"Long-term sigma level: {results['sigma_level_lt']:.2f}σ")
    ...
    # Cpk: 1.330
    # Short-term sigma level: 3.99σ
    # Long-term sigma level: 3.72σ
    # Measured DPMO: 63.0
    # Defective units: 6 / 100000

#### ⚠️ Note: Meaning of 1.5 Sigma Shift

Six Sigma assumes a 1.5σ gap between short-term process capability (Cpk-based) and long-term performance (measured DPMO). This accounts for process drift and special cause variation over time. Therefore, even a 6σ process actually has a DPMO of 3.4.
...
Evaluate repeatability and reproducibility of measurement systems.
    ...
    class GageRnR:
        ...
        def print_report(self):
            ...
            print(f"  Criteria:")
            print(f"    ≥ 5: Sufficient discrimination capability")
    ...
    #   Criteria:
    #     ≥ 5: Sufficient discrimination capability
...
Evaluate both short-term and long-term capability.
    ...
    class ProcessCapabilityBaseline:
        ...
        def print_report(self):
            ...
            print(f"  Short-term sigma level: {r['sigma_level_st']:.2f}σ")
    
            print(f"\n[Long-term Capability Indices (Overall)]")
            print(f"  Pp  = {r['Pp']:.3f}")
            print(f"  Ppk = {r['Ppk']:.3f}  (Ppu={r['Ppu']:.3f}, Ppl={r['Ppl']:.3f})")
            print(f"  Long-term sigma level: {r['sigma_level_lt']:.2f}σ")
    ...
    #   Short-term sigma level: 4.43σ
    #
    # [Long-term Capability Indices (Overall)]
    #   Pp  = 1.339
    #   Ppk = 1.334  (Ppu=1.342, Ppl=1.326)
    #   Long-term sigma level: 4.00σ
    #
    # [Taguchi Index]
...
Identify key factors using multiple regression analysis.
    ...
    class RootCauseAnalyzer:
        ...
        def print_report(self):
            ...
            print(f"  → Factors to manage as priority: {', '.join(r['vital_few'])}")
    ...
    #   → Factors to manage as priority: Temperature, Pressure, Material_Moisture
    # ======================================================================
...
Decompose variation into positional, temporal, and within-piece variation.
    ...
    class MultiVariChart:
        ...
        def plot(self, title: str = "Multi-Vari Chart"):
            ...
            if self.variance_components['lot_pct'] > 50:
                print("  → Lot-to-lot variation is dominant → Process stabilization is top priority")
            elif self.variance_components['position_pct'] > 50:
                print("  → Position-to-position variation is dominant → Equipment uniformity improvement is top priority")
            else:
                print("  → Within-piece variation is dominant → Measurement system improvement is top priority")
    ...
    #
    # Improvement priority:
    #   → Lot-to-lot variation is dominant → Process stabilization is top priority
...
Create a control plan to maintain process after improvements.
    ...
    control_plan.add_control_item(
        ...
        reaction_plan="""
        1. One point outside control limits → Stop line, investigate cause
        2. EWMA outside control limits → Monitor trend, check raw materials
        3. Three consecutive points exceed 2σ → Report to process engineer
        """
    )
    ...
    control_plan.add_control_item(
        ...
        USL=82.0,
        target=80.0,
        measurement_method="Thermocouple (±0.1℃)",
        sample_size=1,
        frequency="Continuous monitoring (1-minute intervals)",
        control_method="Real-time SPC (Shewhart + CUSUM)",
        reaction_plan="""
        1. Outside specification limits → Automatic alarm, adjust heating/cooling
        2. CUSUM signal → Trend analysis, preventive maintenance
        """
    )
    ...
    control_plan.add_control_item(
        ...
        reaction_plan="""
        1. Outside control limits → Add neutralizer, re-measure
        2. Trend detected → Check raw material lot
        """
    )
    ...
    #   Measurement method: Thermocouple (±0.1℃)
...
Track project progress and return on investment.
    ...
    project.set_actual("Sigma Level", 4.75, "σ")
    project.set_actual("Monthly Defects", 120, "units")
    
    # Financial information
    # Initial investment: Equipment improvement, training = 3M yen
    # Annual savings: Defect reduction (350 units × 1,000 yen/unit × 12 months) = 4.2M yen
...
### Basic Understanding
  ...
  * ✅ Explain Six Sigma concept and meaning of sigma levels
  * ✅ Understand and convert between DPMO, yield, and sigma level
  * ✅ Explain meaning and reason for 1.5 sigma shift
  * ✅ Understand DMAIC framework's 5 phases and purpose of each phase

### Practical Skills

  * ✅ Calculate DPMO and sigma level from defect data
  * ✅ Evaluate measurement system validity using Gage R&R analysis
  * ✅ Calculate Cp, Cpk, Pp, Ppk, Cpm and comprehensively assess process capability
  * ✅ Identify root causes with multiple regression analysis and extract Vital Few with Pareto analysis
  * ✅ Decompose variation into positional, temporal, and within-piece components using Multi-Vari chart
  * ✅ Create control plans and sustain improvements
  * ✅ Evaluate economic value of projects with ROI calculation
  * ✅ Find robust parameter settings using Taguchi method

### Application Ability

  * ✅ Plan, execute, and complete Six Sigma projects
  * ✅ Select and apply appropriate statistical tools for each DMAIC phase
  * ✅ Practice complete flow: measurement system analysis → capability analysis → root cause analysis → improvement → control
  * ✅ Evaluate projects from both financial and technical perspectives
  * ✅ Implement noise-resistant process design (robust design)
...
**Explanation** :  
According to the sigma level correspondence table, 5σ has a DPMO of 233. This corresponds to a yield of 99.977%, rated as "good" quality level.
...
NPV = -Initial investment + Σ(Annual CF / (1+discount rate)^year)
...
**Correct Answer** : NPV = approximately 11.3 million yen → Economically viable
...
[← Chapter 2: Statistical Process Control (SPC)](<chapter-2.html>) [Series Table of Contents](<./index.html>)
