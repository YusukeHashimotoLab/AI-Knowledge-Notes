---
title: "Chapter 1: Fundamentals of Audio Signal Processing"
chapter_title: "Chapter 1: Fundamentals of Audio Signal Processing"
subtitle: From Digital Audio to Spectrograms - Understanding and Processing Acoustic Signals
...
By completing this chapter, you will be able to:
  ...
  * ✅ Understand fundamental concepts of digital audio (sampling, quantization)
  * ✅ Implement audio analysis methods in time and frequency domains
  * ✅ Understand the differences between spectrograms and mel-spectrograms
  * ✅ Extract acoustic features such as MFCCs
  * ✅ Build audio processing pipelines using librosa
  * ✅ Implement audio data preprocessing and data augmentation
...
### The Digitization Process
    ...
    ```mermaid
    graph LR
        A[Analog AudioContinuous Signal] --> B[SamplingDiscretization]
        B --> C[QuantizationNumerical Conversion]
        C --> D[Digital AudioNumerical Array]
        D --> E[Audio FileWAV/MP3/FLAC]
...
### 1.4.4 Feature Extraction Pipeline
    ...
    for audio_file in audio_files:
        try:
            ...
            print(f"  ✓ {audio_file}: {len(feats)} features")
...
### 1.5.2 Normalization
    ...
    print(f"RMS normalization: {current_rms:.4f} → {new_rms:.4f}")
...
### Example Processing Pipeline
    ...
    ```mermaid
    graph LR
        A[Audio File] --> B[Loadlibrosa.load]
        B --> C[PreprocessingTrim/Normalize]
        C --> D[Feature ExtractionMFCC/Mel-Spec]
        D --> E[Data AugmentationPitch/Time]
        E --> F[Model InputNumPy Array]
...
In Chapter 2, we will learn the **Fundamentals of Speech Recognition** :
  ...
  * End-to-End models (Wav2Vec, Whisper)
...
**Answer** :
  ...
  * Number of samples: 132,300 samples
  * Maximum frequency: 22,050 Hz (22.05 kHz)
...
$$ m = 2595 \log_{10}\left(1 + \frac{f}{700}\right) $$
     ...
     * Low frequencies (e.g., 100Hz→150Hz): Large mel change
     * High frequencies (e.g., 5000Hz→5100Hz): Small mel change

**Practical Examples** :

  * Speech recognition: Mel-spectrogram → MFCC
  * Music information retrieval: Direct use of mel-spectrogram
  * Speech synthesis: Audio generation from mel-spectrogram (WaveNet, Tacotron)
