---
title: "Chapter 5: Introduction to Object Detection"
chapter_title: "Chapter 5: Introduction to Object Detection"
subtitle: From Image Classification to Object Detection - R-CNN, YOLO, and Modern Methods
...
Image recognition tasks in computer vision are primarily classified into three categories based on their objectives.
    ...
    ```mermaid
    graph LR
        A[Image Recognition Tasks] --> B[ClassificationImage Classification]
        A --> C[DetectionObject Detection]
        A --> D[SegmentationSegmentation]
    
        B --> B1["What is this image?Class label only"]
        C --> C1["What and where?Location + Class label"]
        D --> D1["Which pixel is what?Pixel-level classification"]
...
$$ \text{IoU} = \frac{\text{Area of Overlap}}{\text{Area of Union}} = \frac{|A \cap B|}{|A \cup B|} $$
    ...
    ```mermaid
    graph LR
        A[Predicted Box] --> C[IntersectionOverlap region]
        B[Ground Truth Box] --> C
        C --> D[UnionUnion region]
...
Two-stage detectors perform object detection in two stages: **①region proposal** and **②object classification**.
    ...
    ```mermaid
    graph LR
        A[Input Image] --> B[Stage 1Region Proposal]
        B --> C[Candidate Regions~2000]
        C --> D[Stage 2Classification]
        D --> E[Final Detection ResultsBox + Class]
...
**Fast R-CNN** significantly improved R-CNN's computational efficiency.
    ...
    ```mermaid
    graph LR
        A[Input Image] --> B[CNNFeature Map]
        B --> C[RoI Pooling]
        D[RegionProposals] --> C
        C --> E[FC Layers]
        E --> F1[SoftmaxClassification]
        E --> F2[RegressorBox Regression]
...
**YOLO** formulates object detection as a "regression problem," directly predicting bounding boxes and classes end-to-end with a single CNN.
    ...
    ```mermaid
    graph LR
        A[Input Image448×448] --> B[CNN BackboneFeature Extraction]
        B --> C[Grid Division7×7]
        C --> D[Predict per CellBox + Class]
        D --> E[NMSRemove Duplicates]
...
**NMS** is an algorithm that removes overlapping detection results, keeping only one box per object.
    ...
    ```mermaid
    graph LR
        A[Detection BoxesSort by Score] --> B[Select Highest Score Box]
        B --> C[Remove Overlapping BoxesIoU > threshold]
        C --> D{Boxes Remaining?}
        D -->|Yes| B
