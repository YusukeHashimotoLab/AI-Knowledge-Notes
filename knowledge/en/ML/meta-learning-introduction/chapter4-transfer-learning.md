...
chapter_title: "Chapter 4: Transfer Learning"
subtitle: Efficient learning and domain adaptation techniques using pre-trained models
...
Transfer learning is a machine learning approach that leverages knowledge learned from one task (source task) for another task (target task). While meta-learning aims for "learning to learn", transfer learning aims for "knowledge reuse".
    ...
    ```mermaid
    graph LR
        A[Source DomainImageNet] -->|Pre-training| B[Pre-trained Model]
        B -->|Transfer| C[Target DomainMedical Images]
