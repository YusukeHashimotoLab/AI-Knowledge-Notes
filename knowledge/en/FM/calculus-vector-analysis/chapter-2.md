---
title: "Chapter 2: Fundamentals of Integration and Numerical Integration"
chapter_title: "Chapter 2: Fundamentals of Integration and Numerical Integration"
subtitle: Fundamentals of Integration and Numerical Integration
---
...
<div class="video-container">
  <iframe
    width="560"
    height="315"
    src="https://www.youtube.com/embed/BEElvcgY5Uk?start=904"
    title="Calculus & Vector Analysis Ch.2: Fundamentals of Integration and Numerical Integration"
    frameborder="0"
...
🌐 EN | [🇯🇵 JP](<../../../jp/FM/calculus-vector-analysis/chapter-2.html>) | Last sync: 2025-11-16

[AI Terakoya Top](<../index.html>) > [FM Dojo](<../index.html>) > [Introduction to Calculus and Vector Analysis](<index.html>) > Chapter 2 

## 2.1 Definite Integrals and Indefinite Integrals

**📐 Definition: Definite Integral**  
The definite integral of function f(x) over interval [a, b] is defined as the limit of a Riemann sum: $$\int_a^b f(x) dx = \lim_{n \to \infty} \sum_{i=1}^n f(x_i^*) \Delta x$$ Geometrically, it represents the **signed** area between the curve y = f(x) and the x-axis: parts of the curve above the axis contribute positively, parts below the axis contribute negatively. Only when f(x) ≥ 0 on the whole interval does the definite integral coincide with the ordinary (unsigned) area. 
...
def antiderivative(x):
    """Closed form of ∫ sin(x)·exp(-0.1x) dx"""
    return -np.exp(-0.1*x) * (0.1*np.sin(x) + np.cos(x)) / 1.01

a, b = 0, 20
# Analytical reference value - NOT another numerical result
exact_value = antiderivative(b) - antiderivative(a)

print("SciPy numerical integration:")
print(f"Analytical reference:   {exact_value:.10f}\n")
...
print(f"quad (adaptive):        {result:.10f}, estimated error = {error:.2e}, "
      f"true error = {abs(result-exact_value):.2e}")

# fixed_quad: fixed-order Gauss-Legendre quadrature
result, _ = integrate.fixed_quad(test_function, a, b, n=50)
print(f"fixed_quad (n=50):      {result:.10f}, true error = {abs(result-exact_value):.2e}")

# romb: Romberg integration on 2^k+1 equally spaced samples
# (integrate.romberg was removed in SciPy 1.15; romb is the sample-based replacement)
k = 10
xs = np.linspace(a, b, 2**k + 1)
result = integrate.romb(test_function(xs), dx=(b - a) / 2**k)
print(f"romb (2^10+1 samples):  {result:.10f}, true error = {abs(result-exact_value):.2e}")
...
Analytical reference:   0.9231848794

quad (adaptive):        0.9231848794, estimated error = 5.75e-12, true error = 2.22e-16
fixed_quad (n=50):      0.9231848794, true error = 8.88e-16
romb (2^10+1 samples):  0.9231848794, true error = 0.00e+00
```
...
def copper_heat_capacity(T):
    ...
    T: Temperature [K]
    """
    # Maier-Kelley type linear fit, valid near 300-600 K.
    # (This is NOT the Shomate equation, which uses t = T/1000 and
    #  the form A + B*t + C*t**2 + D*t**3 + E/t**2.)
    A, B = 22.64, 6.28e-3
    return A + B*T
...
```
Heat to heat 1 mol of copper from 300K → 600K:
Numerical integration: Q = 7639.80 J/mol (estimated error: 8.48e-11)
Analytical solution:   Q = 7639.80 J/mol
Difference:            0.00e+00 J/mol
```

For reference, integrating the full NIST Shomate expression for Cu (cr) over the same interval gives 7681.83 J/mol, so this linear fit is about 0.55% low — accurate enough to illustrate the method, but not a substitute for the tabulated thermochemical data.
...
## Summary
  ...
  * Definite integrals represent the signed area between the curve and the axis, used for calculating accumulated quantities
  * Trapezoidal rule has O(h²) accuracy, Simpson's rule has O(h⁴) accuracy as basic numerical integration methods
  * SciPy's quad function achieves high-precision integration through adaptive step size adjustment
  * In materials science, integration is utilized in various situations such as heat calculations and accumulated-quantity analysis
  * Improper integrals and integrals with singularities can also be numerically calculated with appropriate methods

[← Chapter 1: Fundamentals of Differentiation](<chapter-1.html>) [Chapter 3: Multivariable Calculus →](<chapter-3.html>)
