---
title: 第3章：Pythonで体験する機械学習
chapter_title: 第3章：Pythonで体験する機械学習
subtitle: 実践的な予測モデル構築 - 環境構築から実プロジェクトまで
reading_time: 30-40分
difficulty: 中級
code_examples: 35
exercises: 5
...
**Tips:** `requirements.txt`を作成しておくと、環境の再現が簡単になります。 
...
`ValueError: shape mismatch` | データの次元が合わない | `X.shape`と`y.shape`を確認  
`ConvergenceWarning` | 最適化が収束しない | `max_iter`を増やす、またはスケーリング  
...
**5ステップデバッグ手順：**
  ...
  5. **簡単なモデルで試す** : まず`LogisticRegression`で動作確認
...
**ヒント：** `train_test_split`の`test_size`パラメータを変更します。
...
**ヒント：** `accuracy_score`を使って各モデルの精度を計算します。
...
**問題：** ランダムフォレストで`n_estimators=[50, 100, 150]`と`max_depth=[3, 5, 7]`の組み合わせでGrid Searchを実行し、最良のパラメータを見つけてください。

**ヒント：** `GridSearchCV`を使います。
...
## 参考文献
  ...
  2. VanderPlas, J. (2016). _Python Data Science Handbook_. O'Reilly Media.
