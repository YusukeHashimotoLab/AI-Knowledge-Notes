---
title: 第3章：転移学習とファインチューニング
chapter_title: 第3章：転移学習とファインチューニング
subtitle: 事前学習モデルを活用した効率的な学習 - ImageNetからカスタムデータセットまで
reading_time: 20-25分
difficulty: 中級
code_examples: 10
exercises: 5
...
この章を読むことで、以下を習得できます：
  ...
  * ✅ 転移学習の原理とメリットを理解する
  * ✅ ImageNet事前学習モデルの活用方法を習得する
  * ✅ 特徴抽出とファインチューニングの違いを理解し使い分けられる
  * ✅ 段階的な層の解凍とLearning Rate Schedulingを実装できる
  * ✅ PyTorch/torchvisionの事前学習モデルを効果的に活用できる
  * ✅ timmライブラリで最新モデルを使用できる
  * ✅ 実データでの転移学習プロジェクトを完遂できる
...
> 「大規模データセットで学習した特徴抽出器を再利用することで、小規模データセットでも高精度なモデルを構築できる」
    ...
    ```mermaid
    graph LR
        A[ImageNet1.4M画像1000クラス] --> B[事前学習ResNet50]
        B --> C[特徴抽出器汎用的な特徴]
        C --> D[新タスクDogs vs Cats25K画像]
        D --> E[高精度モデル少ないデータで達成]
...
**ImageNet** は画像認識の標準的な大規模データセットです：

  * 約140万枚の画像
  * 1,000のクラスカテゴリ
  * 多様な物体、動物、場面を含む
  * ImageNet Large Scale Visual Recognition Challenge (ILSVRC)で使用
...
### 転移学習の2つのアプローチ
    ...
    graph TD
        ...
        B --> |小規模数百〜数千| C[特徴抽出Feature Extraction]
        B --> |中〜大規模数千〜数万| D[ファインチューニングFine-tuning]
...
**特徴抽出** では、事前学習モデルの畳み込み層を固定し、分類器のみを新しいタスクのために学習します。
...
Kaggleの有名なデータセット「Dogs vs Cats」を使用して、転移学習の完全なワークフローを実装します。
  ...
  * **データセット** : 25,000枚の犬・猫画像
  * **タスク** : 二値分類（犬 or 猫）
  * **手法** : 転移学習（特徴抽出 → ファインチューニング）
  * **評価** : 精度、損失曲線、混同行列
...
**理由** :
  ...
  * データ数が少ない（500枚）ため、ファインチューニングでは過学習のリスクが高い
  * X線画像は自然画像と異なるが、エッジや形状などの低レベル特徴は共通
  * 分類器のみを学習することで、少ないデータでも安定した性能を得られる
  * 計算コストも低く、迅速に結果を得られる
...
**理由** :
  ...
  * データ数が十分に多い（50,000枚）ため、ファインチューニングが有効
  * 製品画像は自然画像と異なる視覚特徴を持つため、深い層の適応が必要
  * 高精度が求められる検査タスクでは、ファインチューニングで性能を最大化
  * 段階的ファインチューニングで過学習を防ぎつつ精度向上
...
**理由** :
  ...
  * データ数は中程度（3,000枚）で、特徴抽出では不十分、全層ファインチューニングでは過学習リスク
  * 衛星画像は自然画像とドメインが異なるため、一部の層の適応が必要
  * 最後のResidualブロック（layer4）と分類器のみを解凍
  * 強い正則化（Dropout、Data Augmentation）と組み合わせる
...
**改善ポイント** :
  ...
  * Stage 1でReduceLROnPlateauを使用し、検証精度が停滞したら学習率を減少
  * Stage 2でCosineAnnealingLRを使用し、学習率を滑らかに減衰
  * Discriminative Learning Ratesで層ごとに異なる学習率を設定
  * 段階的な学習で過学習を防ぎつつ性能を最大化
...
**理由** :
  ...
  * **efficientnet_lite0** : 4.7Mパラメータ、推論時間5ms以下
  * 軽量な設計でモバイルCPU/GPUに最適化
  * 精度も十分（Top-1: 75%前後）
  * エッジデバイスでのリアルタイム処理に適している
...
**理由** :
  ...
  * **convnext_large** : 197Mパラメータ、Top-1: 85%+
  * 最新のCNNアーキテクチャで高精度を実現
  * ImageNet-22Kで事前学習されたバージョンも利用可能
  * 計算コストは高いが、精度を最大化できる
...
**理由** :
  ...
  * **efficientnetv2_rw_s** : 24Mパラメータ、推論時間10ms前後
  * 精度（Top-1: 83%+）と速度の優れたバランス
  * 学習も高速（Progressive Learning採用）
  * 実用的なアプリケーションに最適
...
**注意点** :
  ...
  * 大きな回転は避ける（顔の向きが変わると認識が難しい）
  * 垂直反転は使用しない（上下逆の顔は非現実的）
  * 極端な色調変化は避ける（肌の色は識別の手がかり）
...
**注意点** :
  ...
  * 任意の回転が可能（衛星画像は方向性がない）
  * 垂直反転も有効（地理的な上下は相対的）
  * 強めの明度調整（季節・天候の変化が大きい）
...
**注意点** :
  ...
  * 水平反転は使用しない（文字の左右反転は意味が変わる）
  * 適度なアフィン変換で筆跡のバリエーションをカバー
  * 過度な変形は避ける（文字の判読性を維持）
...
**原因** : ドメインギャップ
  ...
  * 新しいタスクが事前学習タスク（ImageNet）と大きく異なる
  * 事前学習の特徴がタスクに適していない
  * 分類器の表現力が不足している
...
## 参考文献
  ...
  1. Yosinski, J., et al. (2014). "How transferable are features in deep neural networks?" _NIPS_.
  2. He, K., et al. (2016). "Deep Residual Learning for Image Recognition." _CVPR_.
  3. Tan, M., & Le, Q. V. (2019). "EfficientNet: Rethinking Model Scaling for Convolutional Neural Networks." _ICML_.
  4. Dosovitskiy, A., et al. (2021). "An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale." _ICLR_.
  5. Liu, Z., et al. (2022). "A ConvNet for the 2020s." _CVPR_.
  6. Howard, J., & Ruder, S. (2018). "Universal Language Model Fine-tuning for Text Classification." _ACL_.
  7. Smith, L. N. (2018). "A disciplined approach to neural network hyper-parameters." _arXiv:1803.09820_.
  8. Kornblith, S., et al. (2019). "Do Better ImageNet Models Transfer Better?" _CVPR_.
