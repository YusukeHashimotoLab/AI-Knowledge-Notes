---
title: "第5章: 階層ベイズモデルと応用"
chapter_title: "第5章: 階層ベイズモデルと応用"
subtitle: Hierarchical Bayesian Models and Applications
---

[基礎数理道場](<../index.html>) > [推測統計学とベイズ統計](<index.html>) > 第5章 
...
### 5.1.1 階層モデルの利点
  ...
  * **部分プーリング（Partial Pooling）** : 完全プーリング（全データを1つのグループとして扱う）と非プーリング（各グループを独立に扱う）の中間的アプローチ
  * **縮小推定（Shrinkage Estimation）** : 極端な推定値を全体平均に近づけ、過学習を防ぐ
  * **少数データへの対応** : データが少ないグループでも、全体の情報を借用して推定精度を向上
  * **解釈性** : グループ内変動とグループ間変動を分離して理解可能
...
← 前の章: ベイズ推論の基礎とMCMC（準備中） [目次に戻る](<index.html>)
