---
title: 第5章：産業応用
chapter_title: 第5章：産業応用
subtitle: 実プロセスへのベイズ最適化の適用
---
...
#### ✅ Case Study 1の成果
  ...
  * 40回の実験で最適条件を発見 — 初期のラテン超方格サンプリング10点 + BO反復30回（従来のDOE: 150回）
  * 収率: 85% → 94%（+9ポイント、相対では10.6%の向上）
  * 安全制約・品質制約を満たしながら最適化
  * 実験コスト: 73%削減（40回 vs 150回）
...
### Example 3: 精密化学プロセスの微調整
    ...
    # 逐次ベイズ最適化: ラテン超方格20点 + BO反復40回
    print("\nプロセスチューニング実行中（ベイズ最適化: 初期20点 + 反復40回）...\n")
...
#### 💡 ここでBox-Behnken法を実行していない理由

Box-Behnken法は**一括型**の応答曲面計画です。実験を1回も行わないうちに全実験点が確定し、6変数の領域全体で応答が2次関数であることを前提とします。一方、上のループは**逐次型**です。40回の反復のたびにそれまでの全測定値でガウス過程を再学習し、期待改善量が最大となる点に次の実験を配置するため、探索しながら計画そのものが適応していきます。本節が示しているのはこの適応性であり、コード中でBox-Behnken計画は実行しておらず、両者を直接ベンチマークしてもいません。
...
### 主要な学び
  ...
  * **実プロセスの複雑性** : 多目的、制約、高次元など複合的課題
  * **段階的アプローチ** : 簡単な問題から複雑な問題へ
  * **制約ハンドリング** : 安全性・品質規格の確実な遵守
  * **多目的最適化** : トレードオフの可視化と意思決定支援
  * **実験削減効果** : 平均70-80%の実験回数削減
  * **性能向上** : 30-50%の性能改善（ケース依存）
...
← 第4章: 多目的・制約付き最適化（準備中） [目次に戻る](<index.html>)
