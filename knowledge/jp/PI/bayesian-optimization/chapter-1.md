...
**3つの手法はいずれも同じ 30 回の評価予算** で実行しているため、ここで比較しているのは「速さ」ではなく「同じ予算で得られた最良値の良さ」です。これらの目的関数値は負の値（Branin関数の符号を反転して最大化問題にしている）なので、**比をとっても意味がありません**。代わりに最適値までの**残差**で比較します。この探索範囲での最適値は約 -0.40 です：

手法 | 30回の評価での最良値 | 最適値までの残差  
---|---|---  
グリッドサーチ | -12.345 | 11.95  
ランダムサーチ | -8.912 | 8.51  
ベイズ最適化 | -3.456 | 3.06  

**✅ ベイズ最適化の優位性**

  * **評価効率** : 同じ 30 回の予算で、BO はグリッドサーチが残した残差の約 74%、ランダムサーチが残した残差の約 64% を埋めています
  * **予算のスケール** : ここでのグリッドサーチは 1 次元あたり `ceil(30**(1/2)) = 6` 点しか取れません（6×6 格子を 30 点に切り詰めています）。2次元で格子幅を半分にするには評価回数が 4 倍必要ですが、BO はモデルが最も情報量が大きいと判断した点に 1 回ずつ評価を振り向けます
  * **ロバスト性** : 5 回の試行での標準偏差が小さい（安定した性能）
