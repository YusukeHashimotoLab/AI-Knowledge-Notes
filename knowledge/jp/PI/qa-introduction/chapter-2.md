---
title: 第2章：統計的工程管理(SPC)と管理図
chapter_title: 第2章：統計的工程管理(SPC)と管理図
subtitle: プロセス変動を可視化し、異常を早期検知するための実践的手法
---

🌐 JP | [🇬🇧 EN](<../../../en/PI/qa-introduction/chapter-2.html>) | Last sync: 2025-11-16

[AI寺子屋トップ](<../../index.html>)›[プロセス・インフォマティクス](<../../PI/index.html>)›[Qa](<../../PI/qa-introduction/index.html>)›Chapter 2
...
管理図は以下の要素で構成されます：
  ...
  * **中心線(CL: Center Line)** : プロセス平均を示す
  * **上方管理限界(UCL: Upper Control Limit)** : 平均 + 3σ
  * **下方管理限界(LCL: Lower Control Limit)** : 平均 - 3σ
  * **データ点** : 時系列に沿ったプロセス測定値
...
### 基本理解
  ...
  * ✅ SPCの目的と偶然原因・異常原因の違いを説明できる
  * ✅ 管理図の基本構造（CL、UCL、LCL）と3σルールの根拠を理解できる
  * ✅ 計量値管理図（X-bar/R、EWMA、CUSUM）と計数値管理図（P、C）の使い分けができる
  * ✅ 管理限界と規格限界の違いを明確に説明できる

### 実践スキル

  * ✅ X-bar and R管理図を実装し、プロセスの安定性を評価できる
  * ✅ P管理図とC管理図を使って不良率・欠陥数を管理できる
  * ✅ CUSUMとEWMAを実装し、小さな平均シフト（0.5-2σ）を早期検出できる
  * ✅ Western Electric Rulesを適用し、データパターンから異常を検知できる
  * ✅ 管理図とプロセス能力分析を統合し、総合評価ができる
  * ✅ リアルタイム監視システムを構築できる

### 応用力

  * ✅ プロセスの特性に応じて最適な管理図手法を選択できる
  * ✅ 複数の管理図を組み合わせて感度と特異度のバランスを調整できる
  * ✅ 管理図の検出遅延と第一種・第二種過誤のトレードオフを理解し、パラメータ設定できる
  * ✅ 製造現場での実装課題（サンプリング頻度、自動化、アラート疲労）に対処できる
...
[← シリーズ目次へ](<./index.html>) [第3章：シックスシグマとDMAIC →](<./chapter-3.html>)
