---
title: "第1章: なぜ電気化学か"
chapter_title: "第1章: なぜ電気化学か"
subtitle: "電子を導線の中へ流す反応と、カーボンニュートラル社会がそれによって動いている理由"
---
...
🌐 JP | [🇬🇧 EN](<../../../en/MS/electrochemistry-introduction/chapter-1.html>) | Last sync: 2026-08-20

[材料科学道場](<../index.html>) > [電気化学入門](<index.html>) > 第1章
...
本章では語彙を組み立てます。同時に、2026年の材料科学者がなぜこれを気にかけるべきなのかを論じます。これは好みの問題ではありません。カーボンニュートラル社会が依存する技術——水の電気分解、二酸化炭素の変換、そしてあらゆる二次電池——はすべて電気化学的であり、そのすべてが原理ではなく材料によって制限されているからです。計算科学の側から来た方にとって、本シリーズは [OERの計算化学](<../../MI/oer-computational-chemistry/index.html>) と [触媒設計へのMI応用](<../../MI/catalyst-mi-application/index.html>) の下に敷かれた物理的な地盤です。熱力学の側から来た方には、[材料熱力学入門](<../materials-thermodynamics-introduction/index.html>) が、第2章で実際に使うことになる自由エネルギーの道具立てを展開しています。
...
すべての反応が酸化還元というわけではなく、それらを素早く区別できるのは有用な技能です。
  ...
  * **酸化数が変化する。** 両辺のすべての原子に酸化数を割り当てます。どれかが変化していれば、電子が動いています。どれも変化していなければ、その反応は酸化還元ではありません——酸塩基中和とほとんどの沈殿反応がこの範疇に入ります。
  * **単体が現れる、あるいは消える。** 単体（\\(\mathrm{Zn}\\)、\\(\mathrm{Cu}\\)、\\(\mathrm{H_2}\\)、\\(\mathrm{O_2}\\)、\\(\mathrm{Cl_2}\\)）は定義により酸化数がゼロです。単体を消費または生成する反応は、必然的に酸化還元です。
  * **その反応は、原理的には切り分けられる。** これは電気化学者のテストであり、最も有用なものです。電子供与体と電子受容体を物理的に引き離し、導線でつなぐことが想像できるなら、その反応は酸化還元です。できないなら——何かが起こるために両者が触れ合っていなければならないなら——酸化還元ではありません。
...
2つの頭文字合わせが、セルの種類によらず機能します。どちらも極性ではなく化学を指しているからです。
  ...
  * **An Ox** — **An**ode（アノード）、**Ox**idation（酸化）。
  * **Red Cat** — **Red**uction（還元）、**Cat**hode（カソード）。

そして電流の向きに役立つものがもう1つ。これらの語は、そこへ向かって動くイオンと頭文字を共有しています。**An**ion（陰イオン、負のイオン）は **an**ode へ移動し、**cat**ion（陽イオン、正のイオン）は **cat**hode へ移動します。これは両方のセルにおいて成り立ち、化学から導かれます。アノードは溶液中に正電荷を生み出しているので、それを打ち消すために負のイオンが引き寄せられるのです。

これと並んで、第2章までに固めておかなければならない第2の規約があります。かつて流通していた教科書の半分は、逆のやり方をしていたからです。**本シリーズにおける、そして現代の慣行におけるすべての電極電位は、還元電位として表記されます。** すべての半反応は還元の向きに、電子を左辺に置いて書かれ、その表記された電位はその向きを指します。これが **IUPAC規約** であり、何十年ものあいだ標準となってきました。古い「酸化電位」の表に出会ったら、そこにある符号はすべて、私たちが用いるものと逆向きです。第2章はこの規約に全面的に依拠します。セル電圧は \\(E^{\circ}_{\text{cathode}} - E^{\circ}_{\text{anode}}\\) として計算され、両方の値は同じ還元電位表から読み取られ、向きの反転は引き算が引き受けてくれるのです。
...
**水の電気分解。** 水に電流を通すと、水は水素と酸素に分解します。水素は貯蔵可能な燃料であり、化学原料でもあります。窒素と結合すればアンモニアになり、アンモニアは肥料です。熱力学が必要な電圧の下限を定めます——**標準状態で1.23 V**、第2章が水の生成自由エネルギーから導く数値です。実際の電解装置はその下限をかなり上回って運転され、その超過分はほぼ完全に **材料の問題** です。酸素発生電極が遅く、それを高価にも不安定にもせずに速くすることが、この分野の中心的な未解決問題の1つなのです。これはまさに [OERの計算化学](<../../MI/oer-computational-chemistry/index.html>) シリーズの領域です。
...
**結果の読み方。** 重要度の低いものから順に4つ挙げます。
  ...
  * **1アンペアを1時間流すと、およそ1.19グラムの銅が析出します。** これはスケール感として頭に入れておくべき数値です。1アンペアは卓上実験としてはかなりの電流であり、1時間は長い実験であり、それで得られる生成物は1グラム——金属としてティースプーン5分の1ほどです。大きな電極と大きな電流を使わない限り、電気化学が動かす物質量はわずかです。工業的な電気めっきと電解採取が数千アンペアで運転されるのは、まさにこの理由によります。

  * **銅と水素は同じ電荷を消費し、まったく異なる質量を与えます。** どちらも2電子生成物なので、3600 Cはそれぞれ0.018656 molを与えます。しかし銅1モルは63.55 g、水素ガス1モルは2.016 gですから、質量は30倍以上も違います。アルミニウムは逆向きに絵を変えます。原子あたり3個の電子を要するので、同じ電荷では0.012437 molしか得られません。**電子数 \\(n\\) とモル質量は互いに逆方向に引っ張り合っており**、どちらも無視できません。

  * **水素は質量で見れば安く、電荷で見れば高くつきます。** 水素を1キログラム作るには \\(9.57 \times 10^{7}\\) クーロン、およそ26589アンペア時が必要です——銅1キログラムに必要な電荷の30倍以上です。これは特定の技術の欠陥ではありません。化学量論です。水素のモル質量は極めて小さいので、1キログラムは途方もない数のモルに相当し、その1モルごとに電子2モル分の費用がかかるのです。グリーン水素の経済性に関する議論はすべてこの算術から始まり、いかなる触媒の改良もこれを変えることはできません。

  * **ファラデー定数は新しい定数ではありません。** \\(F\\) を電気素量で割ると1モルあたり \\(6.0221 \times 10^{23}\\) が得られ、これはアボガドロ数です。\\(F\\) は単に電子1モル分の電荷であり、ファラデーの法則とは現代の言葉で言えば、電子は1個ずつ数えられるということ、そして化学量論係数が生成物分子1個あたり何個の電子が入るかを教えてくれる、という言明にほかなりません。これらの法則は1830年代に確立されました。電子の存在を誰も知らない数十年前のことです——このことが、正しい定量的法則がその説明に先行した好例の1つとして、科学史における本法則の位置を与えています。
...
[← シリーズトップ](<index.html>) [第2章: 電極電位と熱力学 →](<chapter-2.html>)
