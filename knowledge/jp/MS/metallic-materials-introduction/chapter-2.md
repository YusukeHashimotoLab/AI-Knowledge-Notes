---
title: "Chapter 2: 合金の状態図と相変態"
chapter_title: "Chapter 2: 合金の状態図と相変態"
subtitle: 相律、状態図の読み方、固溶体、金属間化合物、共析・包析反応
---
...
熱力学平衡状態において、系の自由度を決定する基本法則が**Gibbsの相律** です。
...
この反応で生成する組織を**レデブライト（Ledeburite）** と呼びます（鋳鉄に出現）。
...
実際の材料研究では、**CALPHAD（CALculation of PHAse Diagrams）** 法を用いて、熱力学データベースから状態図を計算します。Pythonでは`pycalphad`ライブラリがこの機能を提供しています。

#### pycalphadとは？

`pycalphad`は、多成分系の状態図計算、相平衡計算、熱力学的性質の計算を行うPythonライブラリです。商用ソフトウェア（Thermo-Calc、FactSageなど）と同等の機能をオープンソースで提供します。
...
[← Chapter 1: 金属結合と結晶構造](<./chapter-1.html>) [Chapter 3: 金属材料の強化機構 →](<./chapter-3.html>)
