---
title: "Chapter 4: 金属材料の加工と熱処理"
chapter_title: "Chapter 4: 金属材料の加工と熱処理"
subtitle: Metal Processing and Heat Treatment
---

🌐 JP | [🇬🇧 EN](<../../../en/MS/metallic-materials-introduction/chapter-4.html>) | Last sync: 2025-11-16

# Chapter 4: 金属材料の加工と熱処理

Metal Processing and Heat Treatment

[MS Dojo Top](<../index.html>) > [金属材料入門](<index.html>) > Chapter 4 
...
### 本章の学習目標
  ...
  * 塑性加工（圧延、鍛造、押出し、引抜き）の原理と加工組織の形成を理解する
  * 再結晶・回復・結晶粒成長のメカニズムと焼鈍処理の効果を説明できる
  * 鋼の焼入れ・焼戻しプロセスとマルテンサイト変態を理解する
  * 時効硬化処理（Al合金、ステンレス鋼）の原理と析出挙動を学ぶ
  * TTT図・CCT図を読み取り、冷却条件と得られる組織の関係を予測できる
  * Pythonで変態速度、冷却曲線、時効硬化曲線をシミュレーションできる
...
塑性加工は、材料に降伏応力以上の応力を加えて永久変形させ、所望の形状を得る加工法です。主要な塑性加工プロセスには以下があります：
  ...
  * **圧延（Rolling）** : ロールで材料を圧縮し、板や棒を製造
  * **鍛造（Forging）** : 型で材料を打撃・圧縮し、複雑な形状を成形
  * **押出し（Extrusion）** : ダイスから材料を押し出し、棒や管を製造
  * **引抜き（Drawing）** : ダイスを通して材料を引っ張り、線材を製造
...
**Code Example 1: 加工硬化曲線のシミュレーション**
...
焼鈍（Annealing）は、材料を適切な温度に加熱・保持後、徐冷する熱処理です。主な目的と種類：
  ...
  * **応力除去焼鈍（Stress Relief Annealing）** : 加工や溶接で生じた残留応力を除去（0.3Tm程度）
  * **再結晶焼鈍（Recrystallization Annealing）** : 加工組織を再結晶させて軟化（0.5Tm程度）
  * **完全焼鈍（Full Annealing）** : オーステナイト域まで加熱後、徐冷してフェライト+パーライト組織を得る（鋼）
  * **均質化焼鈍（Homogenization Annealing）** : 鋳造組織の偏析を低減（高温長時間、0.8Tm程度）

### 4.2.2 再結晶速度とJohnson-Mehl-Avrami（JMA）式
...
**Code Example 2: JMA式による再結晶速度シミュレーション**
...
**Code Example 3: Ms温度計算と冷却曲線シミュレーション**
...
時効硬化（Age Hardening, Precipitation Hardening）は、Al合金やステンレス鋼などで用いられる析出強化熱処理です。プロセスは以下の3段階で構成されます：
  ...
  1. **溶体化処理（Solution Treatment）** : 高温（500～550°C、Al-Cu合金の例）で加熱し、溶質元素を固溶させる
  2. **焼入れ（Quenching）** : 急冷して過飽和固溶体を得る
  3. **時効処理（Aging）** : 中温（150～200°C）で保持し、微細な析出物（GP zone → θ''→ θ'→ θ）を形成

析出物が転位の運動を妨げることで、強度が向上します（Orowan機構、Chapter 3参照）。時効時間により、ピーク時効（最大硬度）と過時効（硬度低下）が生じます。

**Code Example 4: 時効硬化曲線のシミュレーション**
...
### 4.5.1 TTT図（Time-Temperature-Transformation Diagram）
...
### 4.5.2 CCT図（Continuous-Cooling-Transformation Diagram）
...
CCT図は、連続冷却時の変態挙動を示した図で、実際の熱処理により近い条件を表します。TTT図と比較して、変態曲線は低温・長時間側にシフトします。冷却曲線がCCT図上の変態曲線と交わる点で、その組織が生成されます。
    ...
    ```mermaid
    graph TD
        A[オーステナイト化850-950°C] --> B{冷却速度}
        B -->|急冷水冷| C[マルテンサイト硬度最大]
        B -->|中間油冷| D[ベイナイト中間硬度]
        B -->|徐冷炉冷| E[フェライト+パーライト軟質]
        C --> F[焼戻し150-650°C]
        F --> G[焼戻しマルテンサイト高強度+靭性]
...
**期待される出力** :
    ...
    Estimated recrystallization temperature: 543 K (270°C)
    At 200°C: Recovery occurs, but NO recrystallization
    

**解説** : 再結晶温度は材料により異なりますが、融点の0.3～0.5倍が目安です。銅の場合、約270°C以上で再結晶が生じます。200°Cでは回復（転位の再配列）のみで、結晶粒の再生成は起こりません。

📝 Exercise 3: Ms温度と焼入れ組織予測 Medium
...
**期待される出力** :
    ...
    Ms ≈ 261°C
    Mf ≈ 46°C
    Water quenching: ~100% Martensite (high hardness, brittle)
    Oil quenching: Martensite + Bainite (mixed, better toughness)
...
📝 Exercise 4: JMA式による再結晶時間の計算 Medium
...
**期待される出力** :
    ...
    Best temperature: 150°C (Max hardness: 140 HV at 50 hours)
    Practical recommendation: 175°C, 15 hours (135 HV, faster processing)
    Over-aging detected at all temperatures (hardness decreases after peak)
    

**解説** : 低温（150°C）ほど最大硬度は高いですが、到達時間が長くなります。実用的には、175°C・15時間が硬度（135 HV）と生産性のバランスが良い条件です。長時間保持すると過時効により硬度が低下するため、適切な時効時間の管理が重要です。

📝 Exercise 6: TTT図を用いた組織予測 Hard
...
**期待される出力** :
    ...
    Scenario A: 100% Pearlite (~20-25 HRC)
    Scenario B: ~100% Martensite (~63-65 HRC)
    Scenario C: 100% Bainite (~40-50 HRC)
...
**[7]** 日本金属学会編 (2015). 『熱処理技術便覧』改訂版. 日本金属学会, pp. 123-189, 267-334. 
...
#### レベル1: 基本理解（知識）
  ...
  * 塑性加工の種類（圧延、鍛造、押出し、引抜き）とその原理を説明できる
  * 加工硬化、回復、再結晶の違いを理解している
  * 焼鈍の種類（応力除去、再結晶、完全、均質化）を列挙できる
  * 鋼の焼入れ・焼戻しプロセスとマルテンサイト変態の基本を理解している
  * 時効硬化の3段階（溶体化、焼入れ、時効）を説明できる
  * TTT図とCCT図の違いと読み方を理解している

#### レベル2: 実践スキル（応用）

  * 加工硬化式\\(\sigma = \sigma_0 + K \varepsilon^n\\)を用いて応力を計算できる
  * JMA式を用いて再結晶速度や変態率を計算できる
  * Ms温度計算式を用いてマルテンサイト変態開始温度を推定できる
  * 冷却曲線をシミュレーションし、焼入れ媒体の影響を評価できる
  * 時効硬化曲線をプロットし、ピーク時効条件を決定できる
  * TTT図を用いて、冷却条件から得られる組織を予測できる
  * Pythonで熱処理シミュレーションコードを作成・実行できる

#### レベル3: 高度な応用（問題解決）

  * 材料の用途に応じて最適な加工・熱処理条件を設計できる
  * 複数の熱処理プロセスを組み合わせた複合処理を計画できる
  * 実験データから加工硬化パラメータや再結晶速度定数を決定できる
  * CCT図を用いて実用的な冷却速度と得られる組織・特性を関連付けられる
  * 時効硬化データから最適な生産条件（温度・時間）を推奨できる
  * 異なる鋼種や合金での熱処理挙動の違いを理解し、予測できる
  * Pythonツールを用いて実験データの解析と最適化を実施できる

**確認方法** : 上記の演習問題（Exercise 1-6）をすべて自力で解けることを確認してください。特に、Exercise 5（時効硬化の最適化）とExercise 6（TTT図による組織予測）は、実用的な問題解決能力を評価する重要な課題です。

[← Chapter 3: 強化機構](<chapter-3.html>) [Chapter 5: Python実践 →](<chapter-5.html>)
