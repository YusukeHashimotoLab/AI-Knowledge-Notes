...
<div class="video-container">
  <iframe
    ...
    title="電池材料設計へのMI応用 第4章: 電池MI実践ケーススタディ"
...
**出典についての注記:** 具体的な文献を挙げている箇所（ケーススタディ3のAttia et al., _Nature_ (2020)）は、その論文の結果です。それ以外の組成や性能数値は、公開されている値の範囲をもとに構成した例示的なワークフロー例であり、特定の実験の測定値ではありません。
...
**例示的な設計例（実測ではありません）:** \- **Li₆.₇₅P₂.₇₅S₁₀.₅Cl₀.₅** : イオン伝導度 2.5 × 10⁻³ S/cm \- Li₇P₃S₁₁の組成最適化版 \- 空気中安定性向上（Cl添加効果）

**検証値（例示）:** \- 予測：2.5 × 10⁻³ S/cm \- 実測：2.1 × 10⁻³ S/cm（誤差16%） \- Li金属との界面抵抗：50 Ω·cm²（目標 < 100）
...
**Li-S電池の利点:** \- 理論容量：1,672 mAh/g（LiCoO₂の理論容量の約6倍） \- 理論エネルギー密度：2,600 Wh/kg \- 硫黄：低コスト、豊富、環境負荷低
...
**例示的な最適化結果（実測ではありません）:** \- メソポーラスカーボン（細孔径 3.5 nm） \- OH官能基密度：2.0 mmol/g \- S担持量：68 wt%

**検証値（例示）:** \- 初期容量：1,350 mAh/g \- 200サイクル後：1,215 mAh/g（90%保持、予測85%） \- クーロン効率：99.2%（目標達成）

**メカニズム:** \- OH官能基がLi₂S_nを化学吸着 \- 適切な細孔径（3-4 nm）で物理的封じ込め \- シャトル効果を大きく抑制

**産業化:** \- エネルギー密度：プロトタイプセルで約400-500 Wh/kgを実証（サイクル寿命は限定的で、量産品ではまだ達成されていません） \- コスト：硫黄は安価で豊富な原料です \- 用途：ドローン、航空機
...
### 4.3.3 実装例
    ...
    class ChargingOptimizer:
        ...
        def step(self, current):
            """1ステップシミュレーション"""
            # 充電
            delta_SOC = current / 60  # Cレートを1分あたりのSOC増加に換算
            self.SOC += delta_SOC
    ...
    def optimize_charging_protocol():
        ...
        protocols = {
            ...
            'Optimized': [6.0]*4 + [5.0]*3 + [4.0]*2 + [2.0]*2  # ML最適化タパー（約10分）
        }
...
**最適充電プロトコル:**
    ...
    Phase 1 (0-40% SOC): 6C充電 = 4.0分（高電流、低温）
    Phase 2 (40-60% SOC): 5C充電 = 2.4分（中電流）
    Phase 3 (60-75% SOC): 4C充電 = 2.25分（電流減）
    Phase 4 (75-80% SOC): 2C充電 = 1.5分（Li析出回避）
    合計：80% SOCまで約10.2分
    

**性能:** \- 充電時間：**約10.2分** （80%到達） \- 最高温度：42°C（安全範囲） \- 劣化速度：1.3%/1000サイクル（従来5%から74%改善）

**実験検証（Stanford University, 2020）:** \- 実際の充電時間：10.2分 \- 850サイクル後：容量保持率 88% \- 公開論文：Attia et al., Nature (2020)。高速充電プロトコルの最適化は産業界でも活発な研究テーマですが、各社が実際に採用しているプロトコルは公開されていません
...
**コバルト問題:** \- 価格：高値かつ変動が大きい（2022年のピークで約$40,000/ton、その後の水準はこれを下回ります） \- 供給：採掘量の約70%がコンゴ民主共和国（地政学リスク） \- 倫理：児童労働、環境破壊

**代替戦略:** \- Ni比率増加：NCM622 → NCM811 → NCM90-05-05 \- Ni利点：高容量（200+ mAh/g）、低コスト

**課題:** \- 高Ni材料の不安定性 \- サイクル性能低下 \- 熱安定性の悪化：Ni比率が上がるほどDSCの発熱開始温度は低下します（NCM622で約250-260°C、NCM811で約210-230°C）。この点がNi比率の上限を決めています
...
### 4.4.3 実装例
    ...
    def evaluate_NCM_composition(x):
        ...
        # 安全性制約: DSC発熱開始温度が210°C以上（Ni比率が上がるほど低下）
...
**例示的な最適組成（実測ではありません）:** \- **LiNi₀.₈₅Co₀.₀₈Mn₀.₀₇O₂** （NCM850807）

**性能:** \- 容量：205 mAh/g \- サイクル寿命：1,200サイクル（80%容量維持） \- 熱安定性：DSC発熱開始温度 約215°C（例示値）。本ケースで設定した210°Cの下限は満たしますが、低NiのNCM622（約250-260°C）よりは低い値です

**Co削減効果:** \- NCM622（Co: 20%）→ NCM850807（Co: 8%） \- Co削減率：60% \- コスト削減：材料費 25%削減

**実用化:** \- 長距離EV：NCM811が量産採用されています \- 超高Ni系（Ni約90%、NCM90-05-05）は2020年代前半から量産されています \- 課題：表面コーティング技術（安定性向上）
...
**最有力候補（既知材料をモデルが再評価したものです）:** \- **Na₃V₂(PO₄)₂F₃** （NASICON構造）
...
Li-S電池 | 細孔径, 官能基密度 | Random Forest | 65% | プロトタイプで約500 Wh/kg（目標）  
