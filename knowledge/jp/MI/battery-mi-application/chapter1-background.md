---
title: 電池材料の基礎とマテリアルズ・インフォマティクスの役割
...
<div class="video-container">
  <iframe
    ...
    title="電池材料設計へのMI応用 第1章: 電池材料の基礎とマテリアルズ・インフォマティクスの役割"
...
**放電時の反応:** \- 負極: `LiC₆ → C₆ + Li⁺ + e⁻` （Li⁺放出） \- 正極: `Li⁺ + e⁻ + CoO₂ → LiCoO₂` （Li⁺挿入） \- 全体: `LiC₆ + CoO₂ → C₆ + LiCoO₂` （E° ≈ 3.7 V）

**充電時** : 逆反応が進行

### 1.1.2 主要性能指標

**エネルギー密度（Energy Density）:** \- 定義：単位質量あたりのエネルギー（Wh/kg） \- 活物質レベルの計算式：`活物質の比エネルギー（Wh/kg） = 容量（mAh/g） × 平均電圧（V）`（活物質だけを対象とした理論値です。たとえば200 mAh/gの材料を3.7 Vで使うと約740 Wh/kgになります） \- セルレベル：実際のセルには負極・電解質・セパレータ・集電体・外装も含まれるため、セルレベルのエネルギー密度は活物質レベルの約3分の1になります \- 現状：セルレベルでLIB 200-300 Wh/kg、目標 500 Wh/kg（全固体電池、セルレベル）
...
**クーロン効率（Coulombic Efficiency）:** \- 定義：`CE = (放電容量) / (充電容量) × 100%` \- 理想値：100%（実際は98-99.5%）
...
**課題5: コスト削減** \- 現状：$150/kWh（2024年） \- 目標：$50/kWh（EVがガソリン車と同等） \- 高コスト要因：CoとLi（2022年のピークでそれぞれ約$40,000/ton、$15,000/ton。その後は下落しており、変動も大きい水準です）
...
**成果例:** \- 10分で80%充電、劣化速度 < 1%/1000サイクル \- EV充電時間：30分 → 10分（Stanford University, 2020） \- 産業的な位置づけ：高速充電プロトコルの最適化は産業界でも活発に研究されている分野ですが、各社が実際に採用しているプロトコルは公開されていません
...
### 学んだこと
  ...
  1. **電池の基礎:** \- 動作原理（酸化還元反応、イオン伝導） \- 主要性能指標（エネルギー密度、サイクル寿命、安全性） \- 電池の種類（LIB、全固体、Li-S、Na-ion）

  2. **開発課題:** \- エネルギー密度向上（300 → 500 Wh/kg） \- 急速充電（10分で80%） \- サイクル寿命延長（500 → 3,000サイクル） \- 安全性向上、コスト削減

  3. **MIの役割:** \- 容量・電圧予測（秒単位） \- サイクル劣化予測（試験期間80%削減） \- 新材料探索（実験70%削減） \- 充電プロトコル最適化

  4. **産業インパクト:** \- 市場規模：$50B（2024年）→ $120B（2030年） \- EV普及によるCO2削減：年間5億トン \- 主要企業：CATL、Panasonic、トヨタ、Tesla
