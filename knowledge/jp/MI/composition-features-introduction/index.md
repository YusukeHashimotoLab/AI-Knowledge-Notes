...
### 推奨学習フロー
    ...
    timeline
        ...
        section Chapter 2 : Magpie詳細
            統計量6種類 : min・max・range・mean・avg_dev・mode
            元素特性22種類から132次元 : Magpie周期表データベース
            実装の数学的定義 : 重み付き統計量
...
#### 学習内容
  ...
  1. **Magpie記述子の数学的定義** \- 132次元の統計量
  2. **統計量6種類** \- minimum、maximum、range、原子分率重み付きmean、avg_dev、mode
  3. **原子分率重み付き平均vs単純平均** \- Magpieのmeanが組成比で重み付けされる理由
  4. **元素特性22種類** \- 電気陰性度、共有結合半径、融点、価電子数・未充填軌道数など
  5. **実装例** \- NumPyによる手動計算
...
Q2: Magpie記述子の132次元は多すぎませんか？過学習の懸念は？

**A:** 実際には問題になりにくいです：

  * 132次元は現代の機械学習では少ない方（GNNは数千次元の埋め込みを学習）
...
**A:** はい、matminerで簡単にできます：
    ...
    # df["composition"]列（Fe2O3など）から自動で132次元ベクトル生成
